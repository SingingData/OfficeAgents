...
def install_to_startup():
    ...
    print("🚀 Installing Weekly Scheduler to Windows Startup")
    print("=" * 50)
    
    # Get current directory
    current_dir = Path.cwd().absolute()
    print(f"📁 Scheduler directory: {current_dir}")
    
    # Check if smart_scheduler.py exists
    scheduler_file = current_dir / "smart_scheduler.py"
    if not scheduler_file.exists():
        print("❌ smart_scheduler.py not found in current directory")
    ...
    try:
        ...
        if venv_path and venv_path != "C:\\path\\to\\your\\venv":
            ...
            print("⚠️  Virtual environment not configured in smart_scheduler.py")
            venv_path = None
    except Exception as e:
        print(f"⚠️  Could not read virtual environment from smart_scheduler.py: {e}")
        venv_path = None
...
def uninstall_from_startup():
    ...
    if batch_file.exists():
        try:
            batch_file.unlink()
            print("✅ Scheduler removed from Windows startup")
            return True
        except Exception as e:
            print(f"❌ Failed to remove startup file: {e}")
            return False
    else:
        print("ℹ️  Scheduler not found in startup folder")
        return True
...
def main():
    ...
    print("🔧 Weekly Scheduler Startup Configuration")
    print("=" * 45)
